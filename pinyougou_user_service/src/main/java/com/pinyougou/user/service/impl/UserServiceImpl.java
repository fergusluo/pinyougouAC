...
public class UserServiceImpl implements UserService {
	...
	public void createSmsCode(String phone) {
		...
		//2、把验证码保存到redis中，key=手机,value=验证码
		BoundValueOperations valOper = redisTemplate.boundValueOps("sms_mobile" + phone);
		//保存验证码
		valOper.set(code);
		//设置有效时间:1分钟
		valOper.expire(1, TimeUnit.MINUTES);
		...
		try {
			...
			map.put("template_code", "SMS_135802012");
			map.put("sign_name", "黑马");
			//{mobile:"",....param:{number:87678}}
	...
	public boolean checkSmsCode(String phone, String code) {
		//读取验证码
		String redisCode = (String) redisTemplate.boundValueOps("sms_mobile" + phone).get();
		//对比验证码
